...
public final class MutableAst {
   ...
   * TODO
   */
  public CelAbstractSyntaxTree replaceSubtreeWithNewBlockCall(String celBlockFunction, CelAbstractSyntaxTree ast, Collection<CelExpr> subexpressions) {
    ...
    return renumberIdsConsecutively(
        CelAbstractSyntaxTree.newParsedAst(blockExpr, ast.getSource()));
  }
   ...
   *       [true].exists(@c0:0, @c0:0)} // Note that i,j gets replaced to the same @c0:0 in this example as they share the same nesting level and type.
   *   <li>{@code [1].exists(i, i > 0) && [1u].exists(j, j > 0u)} -> {@code [1].exists(@c0:0, @c0:0 > 0) && [1u].exists(@c0:1, @c0:1 > 0u)}
   *   <li>{@code [true].exists(i, i && [true].exists(j, j))} -> {@code [true].exists(@c0:0, @c0:0 &&
  ...
  public MangledComprehensionAst mangleComprehensionIdentifierNames(
    ...
    LinkedHashMap<CelNavigableExpr, Pair<CelType, CelType>> comprehensionsToMangle =
        newNavigableAst
            ...
            .filter(node -> {
              ...
              return CelNavigableExpr.fromExpr(node.expr().comprehension().loopStep()).allNodes()
                  .anyMatch(subNode -> subNode.expr().identOrDefault().name().contains(iterVar));
            })
            .collect(Collectors.toMap(k -> k, v -> {
              String iterVar = v.expr().comprehension().iterVar();
              long iterVarId =
                  CelNavigableExpr.fromExpr(v.expr().comprehension().loopStep()).allNodes()
                      .filter(loopStepNode -> loopStepNode.expr().identOrDefault().name().equals(iterVar))
    ...
    return MangledComprehensionAst.of(newNavigableAst.getAst(), ImmutableMap.copyOf(mangledIdentNamesToType));
  }
