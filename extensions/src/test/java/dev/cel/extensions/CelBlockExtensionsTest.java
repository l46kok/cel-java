...
public class CelBlockExtensionsTest {
  private static final Cel CEL =
      CelFactory.standardCelBuilder()
          ...
          .addFunctionDeclarations(
              // These are test only declarations, as the actual function is made internal using @ symbol.
  ...
  public void lazyEval_blockIndexNeverReferenced() throws Exception {
    ...
    CelAbstractSyntaxTree ast =
      compileUsingInternalFunctions("cel.block([get_true()], has(msg.single_int64) ? cel.index(0) : false)");
  ...
  public void lazyEval_blockIndexEvaluatedOnlyOnce() throws Exception {
    ...
    CelRuntime celRuntime =
        CelRuntimeFactory.standardCelRuntimeBuilder()
            ...
            .build();
    CelAbstractSyntaxTree ast =
        compileUsingInternalFunctions("cel.block([get_true()], cel.index(0) && cel.index(0) && cel.index(0))");
  ...
  public void lazyEval_multipleBlockIndices_inResultExpr() throws Exception {
    ...
    CelRuntime celRuntime =
        CelRuntimeFactory.standardCelRuntimeBuilder()
            ...
            .build();
    CelAbstractSyntaxTree ast =
        compileUsingInternalFunctions("cel.block([get_true(), get_true(), get_true()], cel.index(0) && cel.index(0) && cel.index(1) && cel.index(1) && cel.index(2) && cel.index(2))");
  ...
  public void lazyEval_multipleBlockIndices_cascaded() throws Exception {
    ...
    CelRuntime celRuntime =
        CelRuntimeFactory.standardCelRuntimeBuilder()
            ...
            .build();
    CelAbstractSyntaxTree ast =
        compileUsingInternalFunctions("cel.block([get_true(), cel.index(0), cel.index(1)], cel.index(2))");
  ...
  public void blockIndex_invalidArgument_throws() {
    CelValidationException e = assertThrows(CelValidationException.class, () -> compileUsingInternalFunctions("cel.block([1], cel.index(\"a\"))"));

    assertThat(e).hasMessageThat().contains("found no matching overload for 'cel.index'");
  }

  // @Test
  // public void recursive() throws Exception {
  //   CelAbstractSyntaxTree ast = compileUsingInternalFunctions("cel.block([cel.index(1), true, false], cel.index(0) || cel.index(1))");
  //   CelRuntime.Program program = CEL.createProgram(ast);
  //
  //   boolean evaluatedResult = (boolean) program.eval();
  //
