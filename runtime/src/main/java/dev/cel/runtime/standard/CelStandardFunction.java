...
public abstract class CelStandardFunction {
  ...
  public ImmutableSet<CelFunctionBinding> newFunctionBindings(
    ...
    ImmutableSet<CelFunctionBinding> overloadBindings = overloads.stream().map(overload -> overload.newFunctionBinding(celOptions, runtimeEquality)).collect(toImmutableSet());
  ...
  CelStandardFunction(ImmutableSet<CelStandardOverload> overloads) {
    this("func_name_pending", overloads);
  }
