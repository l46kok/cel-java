...
final class EvalVarArgsCall extends PlannedInterpretable {
  ...
  public Object eval(GlobalResolver resolver) throws CelEvaluationException {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      argVals[i] = EvalHelpers.evalStrictly(arg, resolver);
    }
  ...
  static EvalVarArgsCall create(long exprId, CelResolvedOverload resolvedOverload, PlannedInterpretable[] args) {
    return new EvalVarArgsCall(exprId, resolvedOverload, args);
  }

  private EvalVarArgsCall(long exprId, CelResolvedOverload resolvedOverload, PlannedInterpretable[] args) {
    super(exprId);
