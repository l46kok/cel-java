...
final class EvalHelpers {
  ...
  static Object evalStrictly(PlannedInterpretable interpretable, GlobalResolver resolver) {
    try {
      ...
      throw new StrictErrorException(e.getCause(), e.getErrorCode(), interpretable.exprId());
    } catch (Exception e) {
      throw new StrictErrorException(e.getCause(), CelErrorCode.INTERNAL_ERROR, interpretable.exprId());
    }
  }
