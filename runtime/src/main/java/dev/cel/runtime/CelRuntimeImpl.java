...
abstract class CelRuntimeImpl implements CelRuntime {
  ...
  @AutoValue.Builder
  static abstract class Builder implements CelRuntimeBuilder {
    ...
    public Builder setStandardEnvironmentEnabled(boolean value) {
      throw new UnsupportedOperationException("Unsupported. Subset the environment using setStandardFunctions instead.");
    }
    ...
    private static void assertAllowedCelOptions(CelOptions celOptions) {
      ...
      String subsettingError =
              "Subset the environment instead using setStandardFunctions method.";
      ...
      if (!celOptions.enableHeterogeneousNumericComparisons()) {
        throw new IllegalArgumentException(
                prefix + "enableHeterogeneousNumericComparisons cannot be disabled. " + subsettingError);
      }
    }
    ...
    private static DefaultDispatcher newDispatcher(
      ...
      for (CelFunctionBinding binding : standardFunctions.newFunctionBindings(runtimeEquality, options)) {
    ...
    public CelRuntime build() {
      ...
      CelValueProvider protoMessageValueProvider = ProtoMessageValueProvider.newInstance(options(), dynamicProto);
