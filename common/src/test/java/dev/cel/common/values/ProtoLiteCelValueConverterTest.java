...
public class ProtoLiteCelValueConverterTest {
  ...
  private enum UnknownFieldsTestCase {
    INT64(new byte[] {-96, -100, 1, 1}, "2500: 1", ImmutableMap.of(2500, 1L)),
    FIXED32(new byte[] {-83, -100, 1, 2, 0, 0, 0}, "2501: 0x00000002", ImmutableMap.of(2501, 2)),
    FIXED64(
        new byte[] {-79, -100, 1, 3, 0, 0, 0, 0, 0, 0, 0},
        "2502: 0x0000000000000003",
        ImmutableMap.of(2502, 3L)),
    STRING(
        new byte[] {-70, -100, 1, 11, 72, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100},
        "2503: \"Hello world\"",
        ImmutableMap.of(2503, ByteString.copyFromUtf8("Hello world"))),
    REPEATED_INT64(
        new byte[] {-62, -100, 1, 2, 4, 5},
        "2504: \"\\004\\005\"",
        ImmutableMap.of(2504, ByteString.copyFrom(new byte[] {4, 5}))),
    MAP_STRING_INT64(
        new byte[] {
        ...
        "2505: {\n"
            ...
            + "  2: 5\n"
            + "}",
        ImmutableMap.of(2505, ByteString.copyFrom(new byte[] {10, 3, 98, 97, 114, 16, 5}))),
    ;

    private final byte[] bytes;
    private final String formattedOutput;
    private final ImmutableMap<Integer, Object> unknownMap;

    UnknownFieldsTestCase(
        byte[] bytes, String formattedOutput, ImmutableMap<Integer, Object> unknownMap) {
      this.bytes = bytes;
  ...
  public void readAllFields_unknownFieldsWithValues() throws Exception {
    ...
    ImmutableMap<Integer, Object> unknownValues = fields.unknowns();
    assertThat(unknownValues)
        .containsExactly(
            ...
            2505,
            ByteString.copyFrom(new byte[] {10, 3, 98, 97, 114, 16, 5}))
        .inOrder();
  }
}
