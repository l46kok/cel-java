...
final class CelNavigableExprVisitor {
  ...
  private int visit(CelNavigableExpr.Builder navigableExpr, CelCall call) {
    ...
    return Math.max(targetHeight, argumentHeight);
  }
  ...
  private int visit(CelNavigableExpr.Builder navigableExpr, CelComprehension comprehension) {
    int maxHeight = 0;
    maxHeight = Math.max(visit(newNavigableChild(navigableExpr, comprehension.iterRange())), maxHeight);
    maxHeight = Math.max(visit(newNavigableChild(navigableExpr, comprehension.accuInit())), maxHeight);
    maxHeight = Math.max(visit(newNavigableChild(navigableExpr, comprehension.loopCondition())), maxHeight);
    maxHeight = Math.max(visit(newNavigableChild(navigableExpr, comprehension.loopStep())), maxHeight);
    maxHeight = Math.max(visit(newNavigableChild(navigableExpr, comprehension.result())), maxHeight);
  ...
  private int visitStruct(CelNavigableExpr.Builder navigableExpr, CelCreateStruct struct) {
    ...
    for (CelCreateStruct.Entry entry : struct.entries()) {
      ...
      maxHeight = Math.max(visit(value), maxHeight);
    }
    return maxHeight;
  }
  ...
  private int visitMap(CelNavigableExpr.Builder navigableExpr, CelCreateMap map) {
    ...
    for (CelCreateMap.Entry entry : map.entries()) {
      ...
      maxHeight = Math.max(visit(key), maxHeight);
  ...
  private int visitExprList(ImmutableList<CelExpr> createListExpr, CelNavigableExpr.Builder parent) {
    int maxHeight = 0;
    for (CelExpr expr : createListExpr) {
      CelNavigableExpr.Builder arg = newNavigableChild(parent, expr);
      maxHeight = Math.max(visit(arg), maxHeight);
    }
    return maxHeight;
  }

  private CelNavigableExpr.Builder newNavigableChild(CelNavigableExpr.Builder parent, CelExpr expr) {
